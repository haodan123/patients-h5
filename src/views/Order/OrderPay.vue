<template>
  <div class="order-pay-page" v-if="orderPre && address">
    ...
    ></van-submit-bar>
    <cp-pay-sheet
      :pay-callback="`/order/pay/result?id=${orderId}`"
...
onMounted(async () => {
  ...
  // 如果有默认地址就取默认地址,如果没有默认地址就去第一个地址
  if (addRes.data.length) {
    // isDefault 为1的地址代表默认地址
...
// 点击支付
const pay = async () => {
  if (!agree.value) return showToast('请同意协议后再支付')
  if (!address.value) return showToast('请选择收货地址')
  ...
  try {
    const res = await createMedicalOrder({
      ...
      addressId: address.value.id, //地址id
